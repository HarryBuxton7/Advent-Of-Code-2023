...
function main(linesArray) {
  ...
  for (const line of ashArray) {
    ...
    for (let i = 0; i < line.length - 1; i++) {
      ...
      let lowerReflection = line[i];
      let upperReflection = line[i + 1];

      for (let j = 1; j < line.length; j++) {
        if (i - j >= 0 && i + j + 1 <= line.length - 1) {
          lowerReflection += line[i - j];
          upperReflection += line[i + j + 1];
        }
      }
      if (lowerReflection !== upperReflection) {
        let differenceCount = 0;
        for (let i = 0; i < lowerReflection.length; i++) {
          lowerReflection[i] !== upperReflection[i] && differenceCount++;
        }
